package  advanced
{
	...
	public class FireDragon extends Sprite 
	{
		...
		private function onEnterFrame(event : Event) : void 
		{
			...
			{
				canvasBmd.fillRect(new Rectangle(particles[i - 1].x, particles[i - 1].y, 3, 3), color);
			}
		...
		private function boom() : void
		{
			...
			for(var i : int = 300;i > 0;i--)
			{
				...
				var brownForce : BrownForce = new BrownForce(1, 2);
				fireParticle.addForce("brownForce", brownForce);
				var attractionForce : AttractionForce = new AttractionForce(sttractionPoint, 4, 2);
