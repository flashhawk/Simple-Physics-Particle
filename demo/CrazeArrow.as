package 
{
	...
	public class CrazeArrow extends Sprite
	{
		...
		private function loop(e : Event) : void
		{
			...
			while (i-- > 0)
			{
				var angle : Number = ps.particles[i].dir;
				if(angle < 0)
				{
					angle = 360 + angle;
				}
				bmd.copyPixels(bmdArray[int(angle % 360)], arrowBmd.rect, new Point(ps.particles[i].x, ps.particles[i].y));
			}
		...
		public function boom(count : int = 30) : void
		{
			...
			for (var i : int = 0;i < count;i++)
			{
				...
				var brownForce : Brownian = new Brownian(0.5, 1);
